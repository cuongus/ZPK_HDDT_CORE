...
| 29 | Thừa một hàng nút phía trên grid; bấm Back không có tác dụng | GUI status `ZGRID_HDDT` và `ZIN_HDDT_INTEGRATION_F01~pai_0100` | status tạo bằng copy chuẩn danh sách nên Application Toolbar mang theo ~29 mã của list; và mã Back / Exit / Cancel của status đó là `&F03` / `&F15` / `&F12` chứ không phải `BACK` / `EXIT` / `CANC` như `PAI_0100` đang so. Kiểm chứng bằng `SELECT ... FROM rsmptexts WHERE progname = 'ZPG_HDDT_INTEGRATION'` trên S25 client 300 | (1) tạo lại status với type **Normal screen**, Application Toolbar để trống; (2) `PAI_0100` nhận cả ba bộ mã: `BACK`/`&F03`/`RW`, `CANC`/`&F12`/`RE`, `EXIT`/`&F15` |
...
- Mã chức năng Back/Exit/Cancel phụ thuộc cách tạo GUI status; PAI nên nhận cả `BACK`/`&F03`/`RW` để không phụ thuộc.
