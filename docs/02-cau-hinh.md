...
Transaction: **`ZFI_HDDT_CFG`** (nhấn đôi vào bảng để bảo trì).

Thứ tự cấu hình khi triển khai mới đúng bằng thứ tự các mục dưới đây.

---

## 1. `ZFIT_HDDT_PROV` — Danh mục nhà cung cấp

| Trường | Ý nghĩa |
|---|---|
| `PROVIDER` | Mã tự đặt: `VIETTEL`, `FPT`, `VNPT`, `TEMPLATE`… |
| `CLASSNAME` | **Tên lớp adapter**. Phải implement `ZFIIF_HDDT_PROVIDER` và `GET_ID( )` phải trả về đúng `PROVIDER` (engine kiểm tra và báo lỗi rõ nếu lệch) |
| `DESCR` | Mô tả |
| `XACTIVE` | Bỏ tick = khoá nhà cung cấp, không gọi được |

`ZFIR_HDDT_SETUP` nạp sẵn 4 dòng.

---

## 2. `ZFIT_HDDT_CONN` — Kết nối
...
| `BASE_URL` | Dùng khi không muốn tạo destination. Chỉ tới **host** (`https://api-vinvoice.viettel.vn`), phần đường dẫn nằm trong `ZFIT_HDDT_ACT` |
...
## 3. `ZFIT_HDDT_ACT` — Endpoint theo nghiệp vụ
...
| Placeholder | Giá trị |
|---|---|
| `{taxcode}` | `ZFIT_HDDT_CRED-TAXCODE` (MST người bán) |
...
## 4. `ZFIT_HDDT_CRED` — Tài khoản và dải số

| Trường | Ý nghĩa |
|---|---|
| `PROVIDER` / `BUKRS` / `INV_TYPE` | Khoá. `INV_TYPE` để trống = dòng mặc định cho mọi loại hoá đơn |
| `CONNID` | Trỏ tới `ZFIT_HDDT_CONN`. Để trống → lấy kết nối active đầu tiên |
...
## 5. `ZFIT_HDDT_SRC` — Lớp đọc dữ liệu nguồn
...
| `PARM_KEY` | Ý nghĩa |
|---|---|
| `ACTIVE_PROVIDER` | **Nhà cung cấp đang dùng.** Không khai thì core suy từ `ZFIT_HDDT_CRED` nếu công ty chỉ có 1 nhà cung cấp active |
...
Security → điền user/password. Trong `ZFIT_HDDT_CONN` đặt `RFCDEST` và
...
CLASS zcl_my_hddt_vault DEFINITION PUBLIC CREATE PUBLIC.
  PUBLIC SECTION.
    INTERFACES zfiif_hddt_secret.
ENDCLASS.

CLASS zcl_my_hddt_vault IMPLEMENTATION.
  METHOD zfiif_hddt_secret~get_secret.
    " is_cred-seckey là khoá tra cứu; trả về mật khẩu
    rv_secret = ...   " gọi CyberArk / Vault / SSFS của bạn
  ENDMETHOD.
ENDCLASS.
```

Rồi khai `ZFIT_HDDT_PARM: PARM_KEY = SECRET_CLASS, PARM_VAL = ZCL_MY_HDDT_VAULT`.

**(3) Trường `APISECRET`.** Chỉ khi hai cách trên không khả thi. Bắt buộc kèm:

- SE54 → Authorization group riêng cho `ZFIT_HDDT_CRED`, chỉ cấp cho key user.
- Không cấp `S_TABU_DIS` / `S_TABU_NAM` cho bảng này ở PRD ngoài nhóm đó.
- Không cấp `SE16`/`SE16N` trên `ZFIT_HDDT_CRED` cho developer ở PRD.
...
## 8. `ZFIT_HDDT_STAT` — Ánh xạ trạng thái
...
## 9. `ZFIT_HDDT_MAP` — Ánh xạ giá trị
...
## 10. `ZFIT_HDDT_DATE` — Ngày lập hoá đơn
...
## 11. `ZFIT_HDDT_TPL` — Mẫu payload

Chỉ dùng cho `ZFIC_HDDT_PROV_TEMPLATE` và lớp con (`ZFIC_HDDT_PROV_VNPT`).
