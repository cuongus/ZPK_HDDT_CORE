...
### 1.3 Bảng `ZFIT_HDDT_ACT`

Đã có sẵn (do `ZFIR_HDDT_SETUP` nạp) nhưng trỏ URL **UAT** nằm trong `BASE_URL`
của kết nối UAT. Vì `CONNID=PRD` dùng `RFCDEST`, các `API_PATH` tương đối dùng
lại được nguyên vẹn — **không phải sửa gì**.

### 1.4 Bảng `ZFIT_HDDT_CRED`
...
Ánh xạ thuế suất / hình thức thanh toán ở `ZFIT_HDDT_MAP` với `PROVIDER` để
...
1. Đóng sổ nghiệp vụ HĐĐT của ngày cuối cùng với Viettel:
   ZFI_HDDT → lọc s_stat = 00 (chưa tích hợp) → phát hành hết
   → kiểm tra không còn dòng đèn vàng / đỏ

2. Đổi tham số ACTIVE_PROVIDER:
   ZFI_HDDT_CFG → ZFIT_HDDT_PARM
     PROVIDER=(trống)  BUKRS=1000
     PARM_KEY=ACTIVE_PROVIDER   PARM_VAL=FPT

3. Xoá bộ đệm cấu hình của các session đang mở:
   - Người dùng thoát và vào lại ZFI_HDDT, HOẶC
   - ZFI_HDDT_CFG tự gọi ZFIC_HDDT_FACTORY=>RESET( ) sau mỗi lần bảo trì

4. Phát hành 1 hoá đơn thật, kiểm tra:
   - ZFIT_HDDT_INV: PROVIDER = FPT, SERIAL/SEQ có giá trị, STATUS = 40
   - ZFIT_HDDT_LOG: HTTP 200, response có link tra cứu
...
```
ZFIT_HDDT_PARM → ACTIVE_PROVIDER = VIETTEL
ZFIT_HDDT_CRED → dòng VIETTEL: VALID_TO = 31.12.9999
```

Hoá đơn đã phát hành qua FPT vẫn nguyên trong `ZFIT_HDDT_INV` với
`PROVIDER = FPT`, và mọi nghiệp vụ tiếp theo trên các hoá đơn đó (huỷ, điều
chỉnh) vẫn đi qua adapter FPT — vì `ZFIC_HDDT_SERVICE` lấy provider từ request,
...
- [ ] `ZFIT_HDDT_CONN` PROD dùng URL **PROD**, không phải `api-uat…`

### Nghiệp vụ

- [ ] Mẫu số và ký hiệu hoá đơn trong `ZFIT_HDDT_CRED` **khớp thông báo phát
      hành đã được CQT chấp nhận**
- [ ] `ZFIT_HDDT_MAP` `TAXRATE` phủ hết mã thuế `MWSKZ` đang dùng
- [ ] `ZFIT_HDDT_MAP` `PAYMENT` phủ hết `ZLSCH` đang dùng
- [ ] `ZFIT_HDDT_DATE` đúng chính sách kế toán (ngày lập hoá đơn không được nhỏ
...
- [ ] Mật khẩu API **không** nằm trong `ZFIT_HDDT_CRED-APISECRET` ở PRD, hoặc
      bảng đã có authorization group riêng
- [ ] Developer **không** có `SE16`/`SE16N`/`S_TABU_NAM` trên `ZFIT_HDDT_CRED` ở PRD
- [ ] `LOG_PAYLOAD = 'X'` (giữ log để đối chiếu thuế)
- [ ] Đã có kế hoạch lưu trữ / archiving cho `ZFIT_HDDT_LOG` (bảng này lớn nhanh)
