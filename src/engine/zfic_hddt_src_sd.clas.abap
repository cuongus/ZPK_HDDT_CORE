...
*              có chứng từ FI được nguồn FI (ZFIC_HDDT_SRC_FI) xử lý
...
*=====================================================================
CLASS zfic_hddt_src_sd DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    CONSTANTS gc_src_type TYPE zfide_hddt_srctype VALUE 'SD' ##NO_TEXT.
...
*---- 2. Loại billing đã sinh chứng từ FI (nguồn FI xử lý) -----------*
    ...
    LOOP AT lt_vbrk ASSIGNING FIELD-SYMBOL(<ls_vbrk>).
      APPEND VALUE #( sign = 'I' option = 'EQ' low = <ls_vbrk>-vbeln ) TO lt_awkey.
      APPEND VALUE #( sign = 'I' option = 'EQ' low = <ls_vbrk>-vbeln ) TO lt_vbeln.
    ENDLOOP.
    ...
    LOOP AT lt_vbrk ASSIGNING <ls_vbrk>.
      READ TABLE lt_has_fi TRANSPORTING NO FIELDS
           WITH KEY awkey = CONV awkey( <ls_vbrk>-vbeln ) BINARY SEARCH.
      IF sy-subrc = 0.
        DELETE lt_vbrk.
        CONTINUE.
      ENDIF.
      DATA(ls_reg) = registry_of( iv_bukrs    = <ls_vbrk>-bukrs
                                  iv_gjahr    = is_selection-gjahr
                                  iv_src_type = gc_src_type
                                  iv_docno    = CONV #( <ls_vbrk>-vbeln ) ).
      IF keep_document( iv_reversed  = xsdbool( <ls_vbrk>-fksto = 'X' )
                        is_reg       = ls_reg
...
*---- 3. Dòng billing + điều kiện giá --------------------------------*
    ...
    LOOP AT lt_vbrk ASSIGNING <ls_vbrk>.
      APPEND VALUE #( sign = 'I' option = 'EQ' low = <ls_vbrk>-vbeln ) TO lt_vbeln.
      IF <ls_vbrk>-knumv IS NOT INITIAL.
        APPEND VALUE #( sign = 'I' option = 'EQ' low = <ls_vbrk>-knumv ) TO lt_knumv.
      ENDIF.
    ENDLOOP.
    ...
    DATA(lr_kschl) = map_list_as_range( zfiif_hddt_types=>gc_map_type-cond_type ).
    IF lr_kschl IS NOT INITIAL AND lt_knumv IS NOT INITIAL.
      lv_use_cond = abap_true.
      DATA lr_kschl4 TYPE RANGE OF kschl.
      LOOP AT lr_kschl ASSIGNING FIELD-SYMBOL(<ls_k>).
        APPEND VALUE #( sign = <ls_k>-sign option = <ls_k>-option
                        low  = <ls_k>-low(4) ) TO lr_kschl4.
      ENDLOOP.
...
*---- 4. Dựng request ------------------------------------------------*
    LOOP AT lt_vbrk ASSIGNING <ls_vbrk>.
      DATA(lt_doc_vbrp) = VALUE ty_t_vbrp( FOR ls IN lt_vbrp
                                           WHERE ( vbeln = <ls_vbrk>-vbeln ) ( ls ) ).
      DATA(lt_doc_cond) = VALUE ty_t_cond( FOR lc IN lt_cond
                                           WHERE ( knumv = <ls_vbrk>-knumv ) ( lc ) ).
      DATA(ls_req) = build_one( is_vbrk     = <ls_vbrk>
                                it_vbrp     = lt_doc_vbrp
                                it_cond     = lt_doc_cond
                                iv_use_cond = lv_use_cond
                                iv_gjahr    = is_selection-gjahr ).
  ...
  METHOD build_one.
    ...
    rs_request-invoice-header = VALUE #(
      ...
      exch_rate = exch_rate_of( iv_bukrs = is_vbrk-bukrs
                                iv_waers = is_vbrk-waerk
                                iv_kursf = is_vbrk-kurrf )
      inv_date  = config( )->resolve_invoice_date( iv_bukrs = is_vbrk-bukrs
                                                   iv_budat = is_vbrk-fkdat
                                                   iv_bldat = is_vbrk-fkdat
                                                   iv_cpudt = is_vbrk-erdat )
      inv_time  = sy-uzeit ).

*---- Người mua: bên thanh toán (KUNRG) ------------------------------*
    DATA lv_aubel TYPE vbeln.
    READ TABLE it_vbrp INDEX 1 ASSIGNING FIELD-SYMBOL(<ls_first>).
    IF sy-subrc = 0.
      lv_aubel = <ls_first>-aubel.
    ENDIF.
    rs_request-invoice-buyer = read_buyer( iv_kunnr = is_vbrk-kunrg
                                           iv_bukrs = is_vbrk-bukrs
                                           iv_aubel = lv_aubel ).
    IF rs_request-invoice-buyer-ref_no IS NOT INITIAL.
      rs_request-invoice-header-contract_no = rs_request-invoice-buyer-ref_no.
    ENDIF.

    DATA(ls_pay) = payment_of( iv_zlsch = is_vbrk-zlsch iv_bukrs = is_vbrk-bukrs ).
...
*---- Dòng hàng -------------------------------------------------------*
    LOOP AT it_vbrp ASSIGNING FIELD-SYMBOL(<ls_vbrp>).
      ...
      ls_item-item_name = item_name( iv_bukrs   = is_vbrk-bukrs
                                     iv_vbeln   = <ls_vbrp>-aubel
                                     iv_posnr   = <ls_vbrp>-aupos
                                     iv_matnr   = <ls_vbrp>-matnr
                                     iv_default = <ls_vbrp>-arktx ).

      IF iv_use_cond = abap_true.
        " Theo loại điều kiện giá đã cấu hình (AMT+ / AMT- / TAX)
        LOOP AT it_cond ASSIGNING FIELD-SYMBOL(<ls_cond>)
             WHERE kposn = <ls_vbrp>-posnr.
          config( )->map_value( EXPORTING iv_provider  = space
                                          iv_map_type  = zfiif_hddt_types=>gc_map_type-cond_type
                                          iv_sap_value = <ls_cond>-kschl
                                IMPORTING ev_ext_value = DATA(lv_role) ).
          TRANSLATE lv_role TO UPPER CASE.
          CONDENSE lv_role NO-GAPS.
          CASE lv_role.
            WHEN 'AMT+'.
              ls_item-amount = ls_item-amount + <ls_cond>-kwert.
            WHEN 'AMT-'.
              ls_item-amount = ls_item-amount - abs( <ls_cond>-kwert ).
            WHEN 'TAX'.
              ls_item-tax_amount = ls_item-tax_amount + <ls_cond>-kwert.
              IF <ls_cond>-kbetr IS NOT INITIAL.
                ls_item-tax_rate = <ls_cond>-kbetr / 10.
              ENDIF.
            WHEN OTHERS.
          ENDCASE.
        ENDLOOP.
      ELSE.
        ls_item-amount     = <ls_vbrp>-netwr.
        ls_item-tax_amount = <ls_vbrp>-mwsbp.
        ls_item-tax_rate   = tax_rate_of( iv_bukrs = is_vbrk-bukrs
                                          iv_mwskz = <ls_vbrp>-mwskz ).
