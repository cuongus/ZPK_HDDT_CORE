...
CLASS zfic_hddt_token DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS get_token
      IMPORTING io_provider     TYPE REF TO zfiif_hddt_provider
    ...
    METHODS invalidate
      IMPORTING is_conn TYPE zfit_hddt_conn
                is_cred TYPE zfit_hddt_cred .
...
CLASS zfic_hddt_token IMPLEMENTATION.

  METHOD get_token.

    IF iv_force_new = abap_false.
      rv_token = read_cache( is_conn = is_conn
                             is_cred = is_cred ).
    ...
    rv_token = login( io_provider = io_provider
                      is_conn     = is_conn
                      is_cred     = is_cred
                      iv_secret   = iv_secret ).
  ...
  METHOD write_cache.
    ...
    DATA ls_tok TYPE zfit_hddt_tok.
  ...
  METHOD login.
    ...
    IF lv_action IS INITIAL.
      lv_action = zfiif_hddt_types=>gc_action-login.
    ENDIF.

    DATA(ls_act) = lo_config->get_action( iv_provider = is_conn-provider
                                          iv_action   = lv_action ).
    ...
    ls_call-secret    = iv_secret.
    " API đăng nhập không thể dùng Bearer -> ép về Basic
    ls_call-auth_mode = zfiif_hddt_types=>gc_auth-basic.

    ls_call-symbols = VALUE zfiif_hddt_types=>ty_t_kv(
      ( name = zfiif_hddt_types=>gc_symbol-taxcode value = |{ is_cred-taxcode }| )
      ( name = zfiif_hddt_types=>gc_symbol-apiuser value = |{ is_cred-apiuser }| ) ).

    DATA(ls_resp) = NEW zfic_hddt_http( )->send( ls_call ).

    IF ls_resp-http_code < 200 OR ls_resp-http_code >= 300.
      zficx_hddt_error=>raise_text(
        iv_text      = |Đăng nhập { is_conn-provider } thất bại (HTTP | &&
                       |{ ls_resp-http_code } { ls_resp-reason }): { ls_resp-body }|
        iv_http_code = ls_resp-http_code ).
    ENDIF.

    rv_token = io_provider->extract_token( ls_resp-body ).
