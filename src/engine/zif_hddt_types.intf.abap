...
*              VNPT là việc của lớp adapter (ZIF_HDDT_PROVIDER).
...
*---------------------------------------------------------------------*
  ...
  TYPES ty_r_docno TYPE RANGE OF zde_hddt_docno.
...
*---------------------------------------------------------------------*
  TYPES: BEGIN OF ty_seller,
           tax_code   TYPE zde_hddt_taxcode,
           name       TYPE string,
           address    TYPE string,
           phone      TYPE string,
           email      TYPE string,
...
*---------------------------------------------------------------------*
  TYPES: BEGIN OF ty_item,
           ...
           item_type    TYPE zde_hddt_itemtype, " 0 thường 1 KM 2 CKTM 3 ghi chú
...
*---------------------------------------------------------------------*
  TYPES: BEGIN OF ty_adjust,
           adj_type      TYPE zde_hddt_adjtype, " 1/3/5/7
           " '1' = điều chỉnh tăng, '0' = điều chỉnh giảm,
           " '2' = điều chỉnh thông tin (không tiền)
           adj_direction TYPE c LENGTH 1,
           org_serial    TYPE zde_hddt_serial,
           org_seq       TYPE zde_hddt_seq,
           org_inv_date  TYPE dats,
           org_idkey     TYPE zde_hddt_idkey,
...
*---------------------------------------------------------------------*
  TYPES: BEGIN OF ty_header,
           idkey       TYPE zde_hddt_idkey,   " sid / transactionUuid
           inv_type    TYPE zde_hddt_invtype, " 01GTKT / 02GTTT ...
...
*---------------------------------------------------------------------*
  TYPES: BEGIN OF ty_request,
           ...
           " bảng cấu hình ZTB_HDDT_PARM / ZTB_HDDT_CRED
...
*---------------------------------------------------------------------*
  " Mã action là KHOÁ vào bảng ZTB_HDDT_ACT. Mỗi nhà cung cấp khai
  " báo endpoint riêng cho cùng một mã action => core không đổi.
  CONSTANTS: BEGIN OF gc_action,
               login           TYPE zde_hddt_action VALUE 'LOGIN',
               create_invoice  TYPE zde_hddt_action VALUE 'CREATE_INVOICE',
               create_draft    TYPE zde_hddt_action VALUE 'CREATE_DRAFT',
               preview_draft   TYPE zde_hddt_action VALUE 'PREVIEW_DRAFT',
               approve_invoice TYPE zde_hddt_action VALUE 'APPROVE_INVOICE',
               update_invoice  TYPE zde_hddt_action VALUE 'UPDATE_INVOICE',
               replace_invoice TYPE zde_hddt_action VALUE 'REPLACE_INVOICE',
               adjust_invoice  TYPE zde_hddt_action VALUE 'ADJUST_INVOICE',
               cancel_invoice  TYPE zde_hddt_action VALUE 'CANCEL_INVOICE',
               delete_invoice  TYPE zde_hddt_action VALUE 'DELETE_INVOICE',
               search_invoice  TYPE zde_hddt_action VALUE 'SEARCH_INVOICE',
               get_file        TYPE zde_hddt_action VALUE 'GET_FILE',
  ...
  CONSTANTS: BEGIN OF gc_status,
               not_sent   TYPE zde_hddt_status VALUE '00',
               sent       TYPE zde_hddt_status VALUE '10',
  ...
  CONSTANTS: BEGIN OF gc_adj_type,
               ...
               replace  TYPE zde_hddt_adjtype VALUE '3',
               adjust   TYPE zde_hddt_adjtype VALUE '5',
               cancel   TYPE zde_hddt_adjtype VALUE '7',
             END OF gc_adj_type.

  CONSTANTS: BEGIN OF gc_auth,
               basic  TYPE zde_hddt_auth VALUE 'B',
               header TYPE zde_hddt_auth VALUE 'H',
               token  TYPE zde_hddt_auth VALUE 'T',
               oauth2 TYPE zde_hddt_auth VALUE 'O',
               none   TYPE zde_hddt_auth VALUE 'N',
             END OF gc_auth.

  CONSTANTS: BEGIN OF gc_map_type,
               payment   TYPE zde_hddt_maptype VALUE 'PAYMENT',
               tax_rate  TYPE zde_hddt_maptype VALUE 'TAXRATE',
               unit      TYPE zde_hddt_maptype VALUE 'UNIT',
               inv_type  TYPE zde_hddt_maptype VALUE 'INVTYPE',
               item_type TYPE zde_hddt_maptype VALUE 'ITEMTYPE',
               currency  TYPE zde_hddt_maptype VALUE 'CURRENCY',
               gl_acct   TYPE zde_hddt_maptype VALUE 'GLACCT',
               doc_type  TYPE zde_hddt_maptype VALUE 'DOCTYPE',
               " Tầng đọc nguồn (PROVIDER = space):
               tax_code  TYPE zde_hddt_maptype VALUE 'TAXCODE',  " mẫu MWSKZ đầu ra (O*, **)
               tax_acct  TYPE zde_hddt_maptype VALUE 'TAXACCT',  " TK thuế GTGT loại khỏi dòng hàng
               bill_type TYPE zde_hddt_maptype VALUE 'BILLTYPE', " VBRK-FKART được phát hành
               cond_type TYPE zde_hddt_maptype VALUE 'CONDTYPE', " PRCD_ELEMENTS-KSCHL -> AMT+/AMT-/TAX
             END OF gc_map_type.

  " Tên tham số trong ZTB_HDDT_PARM
  CONSTANTS: BEGIN OF gc_parm,
               active_provider TYPE zde_hddt_parmkey VALUE 'ACTIVE_PROVIDER',
               default_connid  TYPE zde_hddt_parmkey VALUE 'DEFAULT_CONNID',
               log_payload     TYPE zde_hddt_parmkey VALUE 'LOG_PAYLOAD',
               local_currency  TYPE zde_hddt_parmkey VALUE 'LOCAL_CURRENCY',
               round_vat       TYPE zde_hddt_parmkey VALUE 'ROUND_VAT',
               seller_name     TYPE zde_hddt_parmkey VALUE 'SELLER_NAME',
               seller_addr     TYPE zde_hddt_parmkey VALUE 'SELLER_ADDR',
               seller_mail     TYPE zde_hddt_parmkey VALUE 'SELLER_MAIL',
               seller_tel      TYPE zde_hddt_parmkey VALUE 'SELLER_TEL',
               seller_bank     TYPE zde_hddt_parmkey VALUE 'SELLER_BANK',
               seller_acct     TYPE zde_hddt_parmkey VALUE 'SELLER_ACCT',
               " --- tầng đọc dữ liệu nguồn (port từ dự án HĐĐT private cloud)
               inv_date_maxback TYPE zde_hddt_parmkey VALUE 'INV_DATE_MAX_BACKDAYS',
               buyer_tax_idtype TYPE zde_hddt_parmkey VALUE 'BUYER_TAX_IDTYPE',
               buyer_id_idtype  TYPE zde_hddt_parmkey VALUE 'BUYER_ID_IDTYPE',
               buyer_name_flds  TYPE zde_hddt_parmkey VALUE 'BUYER_NAME_FIELDS',
               addr_country_sfx TYPE zde_hddt_parmkey VALUE 'ADDR_COUNTRY_SUFFIX',
               exch_rate_factor TYPE zde_hddt_parmkey VALUE 'EXCH_RATE_FACTOR',
               seller_from_t001 TYPE zde_hddt_parmkey VALUE 'SELLER_FROM_T001',
               text_langu       TYPE zde_hddt_parmkey VALUE 'TEXT_LANGU',
               item_text_ids    TYPE zde_hddt_parmkey VALUE 'ITEM_TEXT_IDS',
               item_qty_abs     TYPE zde_hddt_parmkey VALUE 'ITEM_QTY_ABS',
               default_payment  TYPE zde_hddt_parmkey VALUE 'DEFAULT_PAYMENT',
               tax_cond_type    TYPE zde_hddt_parmkey VALUE 'TAX_COND_TYPE',
