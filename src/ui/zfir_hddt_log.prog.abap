*=====================================================================
* Tên/Mã     : ZFIR_HDDT_LOG  (transaction ZFI_HDDT_LOG)
...
*
*              Mật khẩu đã được che NGAY LÚC GHI (ZFIC_HDDT_LOG), nên
...
*=====================================================================
REPORT zfir_hddt_log MESSAGE-ID zfie_hddt.
...
TYPES: BEGIN OF gty_log,
         ...
         test_run    TYPE xfeld,
         http_method TYPE zfide_hddt_method,
         http_code   TYPE zfide_hddt_size,
         http_reason TYPE zfide_hddt_msg,
         duration_ms TYPE zfide_hddt_size,
         serial      TYPE zfide_hddt_serial,
         seq         TYPE zfide_hddt_seq,
         sap_status  TYPE zfide_hddt_status,
         prov_status TYPE zfide_hddt_rccode,
         msgty       TYPE symsgty,
         message     TYPE zfide_hddt_msg,
         req_size    TYPE zfide_hddt_size,
         res_size    TYPE zfide_hddt_size,
         masked      TYPE xfeld,
         cont_type   TYPE zfide_hddt_parmval,
         full_url    TYPE zfide_hddt_url,
         caller      TYPE zfide_hddt_caller,
         tcode       TYPE tcode,
...
DATA gv_docno    TYPE zfide_hddt_docno.
DATA gv_date     TYPE dats.
DATA gv_provider TYPE zfide_hddt_prov.
DATA gv_action   TYPE zfide_hddt_action.
DATA gv_code     TYPE zfide_hddt_size.
...
CLASS lcl_log DEFINITION FINAL CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    METHODS show_part
      IMPORTING iv_part TYPE salv_de_function.
    METHODS export_payload.
    METHODS map_light
      IMPORTING iv_code        TYPE zfide_hddt_size
...
CLASS lcl_log IMPLEMENTATION.
  ...
  METHOD select_data.
    ...
    LOOP AT gt_log ASSIGNING FIELD-SYMBOL(<ls_log>).
      <ls_log>-light = map_light( iv_code  = <ls_log>-http_code
  ...
  METHOD map_light.
    ...
    ELSEIF iv_code >= 200 AND iv_code < 300.
  ...
  METHOD selected_log.
    ...
    IF lv_row < 1 OR lv_row > lines( gt_log ).
      RETURN.
    ENDIF.
    rv_log_id = gt_log[ lv_row ]-log_id.
  ...
  METHOD show_part.
    ...
    DATA(ls_pay) = zfic_hddt_log=>read_payload( lv_log_id ).
    ...
    IF strlen( lv_first ) > 0
      ...
      lt_line = zfic_hddt_json=>pretty( lv_text ).
    ELSE.
...
*&---------------------------------------------------------------------*
FORM show_lines USING iv_title TYPE string
...
*&---------------------------------------------------------------------*
FORM save_raw USING iv_name TYPE string
